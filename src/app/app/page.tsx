...
export default function Home() {
  ...
  const handleDisconnectCouple = async () => {
    ...
    if (!confirm('Tem certeza que deseja desconectar este casal? Todos os devocionais, notas e progresso serão excluídos permanentemente. Esta ação não pode ser desfeita.')) {
      return;
    }

    try {
      const res = await fetch(`/api/couples/${user.coupleId}`, {
        method: 'DELETE',
      });

      if (res.ok) {
        // Atualiza os dados do usuário para refletir a desconexão
        await mutateUser();
      } else {
        const error = await res.json();
        alert(`Erro ao desconectar: ${error.error || 'Erro desconhecido'}`);
      }
    } catch (error) {
      console.error('Error disconnecting couple:', error);
      alert('Erro ao desconectar casal. Tente novamente.');
    }
  };
  ...
  if (user && user.coupleId && !partnerName) {
    return (
        <div className="min-h-screen flex items-center justify-center bg-rose-50 text-center px-4">
            <div className="max-w-md w-full space-y-6">
                <div>
                    <h1 className="text-2xl font-bold mb-4 text-love-900">Aguardando seu parceiro(a) conectar...</h1>
                    <p className="mb-6 text-slate-600">Compartilhe o código do casal com seu parceiro(a) para que ele(a) possa se conectar.</p>
                    {coupleCode && (
                       <div className="bg-white p-6 rounded-xl border border-love-200 inline-block shadow-sm mb-6">
                ...
                <div className="pt-4 border-t border-rose-200">
                    <p className="text-sm text-slate-500 mb-4">
                        Vocês geraram códigos diferentes por engano?
                    </p>
                    <button
                        onClick={handleDisconnectCouple}
                        className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white text-slate-600 border border-slate-300 hover:border-red-300 hover:text-red-600 hover:bg-red-50 transition-colors font-semibold text-sm"
                    >
                        <LogOut size={16} />
                        Desconectar e criar novo código
                    </button>
                </div>
            </div>
        </div>
